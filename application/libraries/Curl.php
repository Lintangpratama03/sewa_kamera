...
class Curl
{
	...
	{
		// Convert the array of options into cURL options
		foreach ($options as $option => $value) {
			if (is_numeric($option)) {
				// If the option is numeric, it means it's a constant (like CURLOPT_XXX)
				// So we directly use it as the option and its value should be in $value
				curl_setopt($this->curl, $option, $value);
			} else {
				// If it's not numeric, it's a CURLOPT_XXX constant name as a string
				// So we need to get the value of the constant and then set the option
				if (defined($option)) {
					curl_setopt($this->curl, constant($option), $value); // Corrected line
				}
			}
		}
